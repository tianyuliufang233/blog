...
helm install higress higress.io/higress -n higress-system --create-namespace --set=global.local=true --set=higress-core.gateway.replicas=1 --set=higress-console.pvc.rwxSupported=false  --set=higress-core.gateway.hostNetwork
```
...
- kubernetes 集群ip初始化后，需要删除集群service重新初始化，才能重新生成cluster ip。
- calico初始化不了，看看init容器日志。
